...
def main():
    ...
    for i, max_freq in enumerate(max_freq_list):
        radio1.set_register('MAX_FREQ', max_freq)
        radio2.set_register('MAX_FREQ', max_freq)
        for j, min_freq in enumerate(min_freq_list):
            if min_freq >= max_freq:
                break
            radio1.set_register('MIN_FREQ', min_freq)
            radio2.set_register('MIN_FREQ', min_freq)
            radio1.reboot_radio()
            radio2.reboot_radio()
            radio1.init_modem()
            radio2.init_modem()
            ...
            if ex_found > 0:
                ...
                results.append('FAIL')
            ID.append(datetime.datetime.now().strftime('%d/%m-%H:%M:%S'))
            min_freq_param.append(min_freq)
            max_freq_param.append(max_freq)
    radio1.multithread_read_shutdown()
    radio2.multithread_read_shutdown()
    common_utils.close_all_serial(serial_port_list)
    ########################################################
    modem_data_list = [
        ['MAX_FREQ', modem_params_dict.get('MAX_FREQ')[-1], max_freq_param],
        ['MIN_FREQ', modem_params_dict.get('MIN_FREQ')[-1], min_freq_param]
    ]
